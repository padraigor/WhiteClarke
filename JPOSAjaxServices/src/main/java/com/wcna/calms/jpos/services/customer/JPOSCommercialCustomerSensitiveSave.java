...
public class JPOSCommercialCustomerSensitiveSave extends JPOSCommercialCustomerSave {

	private final IApplicationService applicationService;
	private final IJPOSAppStatusService jposApplicationService;
	
	public JPOSCommercialCustomerSensitiveSave(
			...
			if (refId == 0) {
				...
				if (!isProposalPastWorking || isConfirmedSensitive) {
					...
					if (isConfirmedSensitive) {
						// revert application status back to working
